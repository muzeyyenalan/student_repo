...
resource "null_resource" "config" {
  ...
  connection {
    ...
    type = "ssh"
    user = "ec2-user"
    private_key = file("./FatalSecKey.pem")
  ...
  provisioner "file" {
    # Do not forget to define your key file path correctly!
    source = "./FatalSecKey.pem"
    destination = "/home/ec2-user/FatalSecKey.pem"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo node1 ansible_host=${aws_instance.nodes[1].private_ip} ansible_ssh_private_key_file=~/FatalSecKey.pem ansible_user=ec2-user >> inventory.txt",
      "echo node2 ansible_host=${aws_instance.nodes[2].private_ip} ansible_ssh_private_key_file=~/FatalSecKey.pem ansible_user=ec2-user >> inventory.txt",
      "echo [ubuntuservers] >> inventory.txt",
      "echo node3 ansible_host=${aws_instance.nodes[3].private_ip} ansible_ssh_private_key_file=~/FatalSecKey.pem ansible_user=ubuntu >> inventory.txt",
      "chmod 400 FatalSecKey.pem"
    ]
  }
}
